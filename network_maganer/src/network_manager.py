...
def gui():
	...
	while True:
		...
		if event == '_LIST_' and len(values['_LIST_']): 
			...
			idx = int(r[0])-1
			window.Element('_STATUSTAB_').update(title='Node ' + str(idx+1) + ' Status')
			window.Element('_STATSTAB_').update(title='Node ' + str(idx+1) + ' Stats')
			updateTabs(idx)
		if event == '_RSNET_':
			active_nodes = 0
		...
		if(len(line) > 4):
			...
			if line == "Startup complete":
				...
				try:
					...
					bat = nodes[int(msg['nID'])-1]['battery'] * float(t_packets-1)/t_packets + float(msg['VBAT']) * float(1/t_packets)
					...
					if((int(msg['nID']) is not 255) and (msg['f'] == 's')):
					...
					if((int(msg['nID']) is not 255) and (msg['f'] == 'u')):
						...
						window.Element('_ACTIVENODES_').update(value=str(active_nodes))
						updateTabs(nidx)
					if((int(msg['nID']) is not 255) and (msg['f'] == 'a')):
